...
for file in os.listdir(directory):
    ...
    #convert to day type
    end = _input["End Time"].values.astype('datetime64[D]')
    ship = _input["Shipment Date"].values.astype('datetime64[D]')
    # create clean df
    delta = np.busday_count(ship, end) # business days
    ...
    training.to_csv("/Volumes/Capstone/bdaytest/" + filename + '.csv', index=False)
