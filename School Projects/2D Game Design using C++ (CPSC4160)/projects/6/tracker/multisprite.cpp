...
Vector2f MultiSprite::makePosition(int px, int py) {
  float newpx = Gamedata::getInstance().getRandFloat(px-300,px+300);;
  float newpy = py;//Gamedata::getInstance().getRandFloat(py-250,py+250);;

  newpx = std::max(newpx,30.0f);
  newpx = std::min(newpx,0.8f * ((float)Gamedata::getInstance().getXmlInt("world/width")));
  //newpy = std::max(newpy,30.0f);
  //newpy = std::min(newpy,((float)Gamedata::getInstance().getXmlInt("world/height"))/2.0f);
